...
ge::graphStatus InferShapeSparseFlashAttention(gert::InferShapeContext *context)
{
    ...
    *attentionOutShape = *queryShape;
    ...
    const bool *lseFlagPtr = attrs->GetAttrPointer<bool>(RETURN_SOFTMAX_LSE_ATTR_INDEX);
    bool returnSoftmaxLse = (lseFlagPtr != nullptr) ? *lseFlagPtr : false;

    if (returnSoftmaxLse) {
        // queryShape 维度 == 3 → layout_query 是 TND；== 4 → BSND
        if (queryShape->GetDimNum() == DIM_NUM_3) {
            // TND: 输出 [N2, T1, G]
            // key 的 N2 位置：PA_BSND = dim 2，BSND/TND = dim 1
