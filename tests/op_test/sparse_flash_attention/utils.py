...
def sfa_run_npu(test_data, testcase_name=None, device_id=0, result_path=None):
    ...
    try:
        ...
        # vllm-ascend baseline 阶段算子只返回单个 attention_out tensor
        # 步骤 2 完成 lse 后会返回 (attn_out, softmax_max, softmax_sum) tuple
        if isinstance(npu_result, (tuple, list)):
            npu_attn_out = npu_result[0]
        else:
            npu_attn_out = npu_result
        print("[compare] comparing attn_out ...")
        result_attn_out, fulfill_percent_attn = result_compare_method.check_result(cpu_result[0], npu_attn_out)
